...
export default function WorkProgress({progress,run,runs,open,onOpen,onSelect,onAction,onEvidence,onCurrent}:{progress:Progress;run?:PiRun;runs:PiRun[];open:boolean;onOpen:(open:boolean)=>void;onSelect:(id:string)=>void;onAction:()=>void;onEvidence:()=>void;onCurrent:()=>void}){
 const [all,setAll]=useState(false);
 useEffect(()=>setAll(false),[run?.id]);
 if(!run&&!open)return null;
 const rows=all?progress.activities:progress.activities.slice(-6);
 return <section className={`work-progress ${progress.tone}`} aria-label={progress.historical?'历史工作进展':'当前工作进展'} data-motion={progress.animate?'on':'off'}>
  ...
  <details open={open} onToggle={e=>onOpen(e.currentTarget.open)}><summary>工作轨迹<span>{open?'收起':'展开'}</span></summary>
   <div className="work-progress-body">
    {run&&<div className="work-progress-tools"><label>查看范围 <select aria-label="工作进展轮次" value={run.id} onChange={e=>onSelect(e.target.value)}>{runs.map((r,i)=><option key={r.id} value={r.id}>{i===0?'当前轮':'历史轮次'} · {nodeLabels[r.stage]||'需求处理'} · {new Date(r.created*1000).toLocaleString('zh-CN')}</option>)}</select></label></div>}
