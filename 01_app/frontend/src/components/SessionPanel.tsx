...
export default function SessionPanel({sessions,sid,search,onSearch,archived,open,busy,running,onToggle,onNew,onChoose,onArchive,onDelete,onArchived}:Props){
 return <aside className={`session-panel ${open?'':'sessions-collapsed'}`}>
  <div className="session-heading"><Button type="text" aria-label={open?'收起会话列表':'展开会话列表'} aria-expanded={open} onClick={onToggle}>会话</Button><Button type="text" aria-label="新建会话" disabled={busy} icon={<PlusOutlined/>} onClick={onNew}/></div>
  <Input aria-label="搜索会话" prefix={<SearchOutlined/>} placeholder="搜索会话名称" value={search} onChange={e=>onSearch(e.target.value)}/>
  <div className="session-list">{sessions.filter(s=>s.title.toLowerCase().includes(search.toLowerCase())).map(s=><div key={s.id} className={`session-item ${sid===s.id?'selected':''}`}><button className="session-select" disabled={busy} onClick={()=>onChoose(s.id)} aria-current={sid===s.id?'true':undefined} title={`${s.title} · ${new Date(s.updated*1000).toLocaleString('zh-CN')}`}><MessageOutlined/><strong>{s.title}</strong></button><span className="session-row-actions">{archived?<><Button type="text" size="small" aria-label={`恢复会话：${s.title}`} disabled={busy} onClick={()=>onArchive(s,false)}>恢复</Button><Button type="text" size="small" danger icon={<DeleteOutlined/>} aria-label={`删除会话：${s.title}`} disabled={busy} onClick={()=>onDelete(s)}/></>:<Button type="text" size="small" icon={<InboxOutlined/>} aria-label={`归档会话：${s.title}`} disabled={busy||running(s.id)} onClick={()=>onArchive(s,true)}/>}</span></div>)}{!sessions.length&&<p className="muted session-empty">{archived?'没有已归档会话':'直接输入，开始第一段会话'}</p>}</div>
