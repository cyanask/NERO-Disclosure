...
export interface ReadingPosition {messageId?:string;runId?:string;blockIndex:number;delta:number;relative:number}
...
export function captureReadingPosition(panel:HTMLElement,messages:HTMLElement):ReadingPosition|undefined{
 ...
 return {messageId:owner?.dataset.messageId,runId:owner?.dataset.runId,blockIndex:owner&&block?Array.from(owner.querySelectorAll(blocks)).indexOf(block):0,delta:block?block.getBoundingClientRect().top-line:0,relative:line-rect.top};
}
