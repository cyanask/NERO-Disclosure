...
function CompanyWorkbench({company,onCompanySettings}:{company:Company;onCompanySettings:()=>void}){
 ...
 const eventList=<><div className="gov-toolbar"><div><h2>事项办理记录</h2><p>按业务事项跟踪当前阶段、修订版本和办理详情。</p></div><Button loading={busy} onClick={refreshEvents}>刷新事项</Button></div><Table rowKey="id" dataSource={records} pagination={{current:eventPage,onChange:setEventPage}} loading={busy} scroll={{x:760}} columns={[{title:'事项',dataIndex:'title',render:(v,r)=><Button type="link" className="text-link" onClick={()=>open(r.id)}>{v}</Button>},{title:'公司',dataIndex:'company_name',width:180},{title:'当前阶段',dataIndex:'stage',width:170,render:v=><span className="status-text">{stages[v]||v}</span>},{title:'版本',dataIndex:'revision',width:70,render:v=>`r${v}`},{title:'更新时间',dataIndex:'updated_at',width:165,render:v=>new Date(v).toLocaleString('zh-CN')}]} locale={{emptyText:<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span>暂无事项记录。请在对话操作台办理本公司事项。<br/><Button type="link" onClick={()=>navigate('console')}>前往对话操作台</Button></span>}/>}}/></>;
