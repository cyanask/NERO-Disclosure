...
class ResearchSession:
    def __init__(self):self.lock=threading.Lock();self.version=None;self.cache={};self.seen=set();self.streak=0;self.stats={'searches':0,'empty_searches':0,'cache_hits':0,'suppressed_searches':0}
    ...
    def read(self,name,args,version,call):
        ...
        result=call()
        with self.lock:
            # A slower result from an earlier source version must not overwrite
            # the cache established by a newer concurrent read.
            if version!=self.version:return result
            if search:
